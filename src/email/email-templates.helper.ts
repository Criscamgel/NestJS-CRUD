...
// ─────────────────────────────────────────────
// Template: Appointment — confirmación de cita al cliente
// ─────────────────────────────────────────────
export function appointmentConfirmationEmailTemplate(payload: {
  name: string;
  date: string;
  time: string;
  duration: number;
  ...
  const showLogo = existsSync(getEmailLogoPath());
  const { name, date, time, duration, meetingLink } = payload;

  const row = (label: string, value: string) => `
<tr>
  <td style="padding: 10px 0; border-bottom: 1px solid #E5E7EB; font-size: 14px; color: ${BRAND.textMuted}; width: 35%;">${label}</td>
  <td style="padding: 10px 0; border-bottom: 1px solid #E5E7EB; font-size: 14px; color: ${BRAND.textBody}; font-weight: 600;">${escapeHtmlForEmail(value)}</td>
</tr>`;

  const content = `
    <h2 style="margin: 0 0 8px; color: ${BRAND.neutral}; font-size: 22px; font-weight: 700;">
      ¡Tu cita con Cheky está confirmada!
    </h2>
    <p style="margin: 0 0 20px; color: ${BRAND.textBody}; font-size: 15px; line-height: 1.7;">
      Hola <strong>${escapeHtmlForEmail(name)}</strong>, tu reunión con el equipo de Cheky ha sido agendada exitosamente.
    </p>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin: 0 0 24px;">
      ${row('Fecha', date)}
      ${row('Hora', time)}
      ${row('Duración', `${duration} minutos`)}
      ${row('Videollamada', `<a href="${meetingLink}" style="color: ${BRAND.primary}; text-decoration: underline;">${meetingLink}</a>`)}
    </table>

    ${ctaButton('Unirme a la reunión', meetingLink)}

    ${infoBox('Ingresa al link de la videollamada a la hora indicada. Si necesitas reprogramar, escríbenos a <a href="mailto:ventas@cheky.co" style="color:' + BRAND.primary + ';">ventas@cheky.co</a>.')}
  `;
...
// ─────────────────────────────────────────────
// Template: Appointment — notificación a ventas
// ─────────────────────────────────────────────
export function appointmentSalesNotificationEmailTemplate(payload: {
  name: string;
  email: string;
  company: string;
  phone: string;
  ...
  const row = (label: string, value: string) => `
<tr>
  <td style="padding: 12px 0; border-bottom: 1px solid #E5E7EB; font-size: 14px; color: ${BRAND.textBody};">
    <strong style="display:block; color: ${BRAND.neutral}; font-size: 12px; text-transform: uppercase; letter-spacing: 0.04em; margin-bottom: 4px;">${label}</strong>
    <span style="word-break: break-word;">${escapeHtml(value)}</span>
  </td>
</tr>`;
  ...
  const content = `
    <h2 style="margin: 0 0 12px; color: ${BRAND.neutral}; font-size: 22px; font-weight: 700;">
      Nueva cita agendada 📅
    </h2>
    <p style="margin: 0 0 20px; color: ${BRAND.textBody}; font-size: 15px; line-height: 1.7;">
      Un visitante agendó una reunión desde la landing.
    </p>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin: 0 0 24px;">
      ...
      ${row('Fecha', payload.date)}
      ${row('Hora', payload.time)}
      ${row('Duración', `${payload.duration} min`)}
      ${row('Link', payload.meetingLink)}
    </table>

    ${ctaButton('Unirme a la reunión', payload.meetingLink)}
  `;
