...
export class PokemonService {
  ...
  async findAll({ limit = this.defaultLimit, offset = 0 }) {
  ...
  async findOne(term: string) {
    ...
    // MongoID
    if( !pokemon && isValidObjectId(term) ) pokemon = await this.pokemonModel.findOne({ _id: term });
    // Name
  ...
  async remove(id: string) {
    ...
    return `Pokemon ${ id } has to be delete correct`;
  }
